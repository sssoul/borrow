        ...
        if ($request->initSession() && $request->isSafe() && $login = Login::isMember()) {
            if (Login::checkPermission($login, 'can_config') && Login::notDemoMode($login)) {
                try {
                    ...
                    foreach (['show_title_logo', 'new_line_title'] as $key) {
                    ...
                    if ($request->post('theme_option')->toBoolean()) {
                        ...
                        // ใช้ค่าที่ส่งมา
                        foreach (['header_bg_color', 'warpper_bg_color', 'content_bg', 'header_color', 'footer_color', 'logo_color'] as $key) {
                        ...
                        foreach ($request->getUploadedFiles() as $item => $file) {
                            if (preg_match('/^file_(logo|bg_image)$/', $item, $match)) {
                                ...
                                if (!File::makeDirectory($dir)) {
                                    ...
                                    if (!$file->validFileExt(self::$cfg->img_typies)) {
