...
class Model extends \Kotchasan\Model
{
    ...
    public static function get(Request $request, Controller $controller)
    {
        ...
        if ($id > 0) {
            // อ่านจาก ID
            $language = static::createQuery()->from('language')->where(['id', $id])->first();
            if ($language) {
                $language = self::extract($language);
            }
        } elseif ($key != '') {
            // อ่านจาก key
            $language = static::createQuery()->from('language')->where(['key', $key])->first();
            if ($language) {
                $language = self::extract($language);
            }
                ...
                try {
                    ...
                    if (empty($values)) {
                        ...
                        if ($save['js']) {
                            ...
                            $save['key'] = strtoupper(preg_replace(['/[\s]+/', '/([^A-Z_]+)/i'], ['_', ''], $save['key']));
                        }
                        ...
                        if ($id > 0 && !$language) {
                            ...
                            // ตรวจสอบข้อมูลซ้ำ
                            $search = $model->db()->first($table_language, [
                                ['key', $save['key']],
                                ['js', $save['js']]
                            ]);
                            if ($search && ($id == 0 || $id != $search->id)) {
                                $ret['ret_write_key'] = Language::replace('This :name already exist', [':name' => Language::get('Key')]);
                            } else {
                                // บันทึก
                                if ($id == 0) {
                                    // ใหม่
                                    ...
                                    // redirect
                                    $ret['location'] = $request->getUri()->postBack('index.php', ['module' => 'language', 'js' => $save['js'], 'sort' => 'id DESC']).'#datatable_'.$id;
                                } else {
                                    // แก้ไข
                                    $model->db()->update($table_language, $id, $save);
                                    // redirect
                                    $ret['location'] = $request->getUri()->postBack('index.php', ['module' => 'language', 'js' => $save['js']]).'#datatable_'.$id;
                                }
                                // อัปเดตไฟล์ ภาษา
