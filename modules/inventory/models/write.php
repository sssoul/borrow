    ...
    public static function get($id)
    {
        if (empty($id)) {
            ...
            $query = static::createQuery()
                ...
                ->where(['V.id', $id]);
            $select = ['V.*', 'I.product_no', 'I.unit'];
            $n = 1;
            foreach (Language::get('INVENTORY_METAS', []) as $key => $label) {
                $query->join('inventory_meta M'.$n, 'LEFT', [['M'.$n.'.inventory_id', 'V.id'], ['M'.$n.'.name', $key]]);
                    ...
                    if ($index) {
                        ...
                        if ($index->id == 0) {
                            ...
                            if ($items['product_no'] == '') {
                                ...
                                $search = $db->first($inventory_items, ['product_no', $items['product_no']]);
