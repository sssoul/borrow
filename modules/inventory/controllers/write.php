    ...
    public function render(Request $request)
    {
        ...
        if (Login::checkPermission($login, 'can_manage_inventory')) {
            ...
            if ($product) {
                ...
                $inline = $header->add('div', [
                    'class' => 'inline'
                ]);
                $writetab = $inline->add('div', [
                    'class' => 'writetab'
                ]);
                $ul = $writetab->add('ul', [
                    'id' => 'accordient_menu'
                ...
                $tabs = [
                    'product' => ['{LNG_Inventory}', 'Inventory\Write\View']
                ];
                if ($product->id > 0) {
                    $tabs['items'] = ['{LNG_Serial/Registration No.}', 'Inventory\Items\View'];
                }
